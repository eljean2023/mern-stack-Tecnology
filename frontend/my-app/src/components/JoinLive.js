...
const JoinLive = () => {
   ...
   // var [enabledStream, setenbledStream] = useState(localStorage.getItem("enabledStream") || "")
    ...
    const url = window.location.pathname + `/live/${id}`;
...
const testOne = () => {
  ...
  return (
    ...
    <form onSubmit={(e)=>{e.preventDefault()}}>
        ...
        value={userNamex}
        />
        <button onClick={joinLive} className="btn btn-secondary checkCopy" hidden={userNamex.length<1}>Go</button>
        ...
        <button onClick={joinLive} className="btn btn-secondary checkCopy" hidden={userNamex.length<1}> </button>
        {/*<button onClick={ClickNotification} className="btn btn-secondary" >Notify</button>*/}
        </div>
        </div>
